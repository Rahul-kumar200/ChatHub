...
const ConversationArea = () => {
  ...
  const sendMessage = () => {
    if(currentMessage.trim()!=='' && currentRoom.roomId.trim()!==''){
        ...
        setCurrentRoom({roomId : currentRoom.roomId , newMessage: 0 , chats :[...obj , {name :'You',type:0,chat :currentMessage}]}); 
    }
    setCurrentMessage('')
  };
  ...
  socket.on('receiveMessage',(msg,roomId,name)=>{
    if(roomId===currentRoom.roomId){
        let obj = currentRoom.chats;
        setCurrentRoom({roomId : currentRoom.roomId , newMessage: 0 , chats :[...obj , {name :name ,type:1,chat :msg}]}); 
        ...
        for(let room of dataAtom.rooms){
            if(room.roomId===roomId){
                ...
                obj.push({roomId:roomId , newMessage : room.newMessage+1 , chats:[...prev_chats,{name:name,type:1,chat:msg}]});
            }
            else{
                obj.push(room);
            }
        }
        setCurrentRoom({roomId : currentRoom.roomId , newMessage: 0 , chats : currentRoom.chats}); 
